...
public class MyUI extends UI {
	...
	private void buildMainPage() {
		...
		HorizontalLayout header = addHeader();
	...
	private void setNewQuestionSite(QuestionNodeData successorNodeData) {
		...
		if (successorId == -1) {
			Page.getCurrent().open("http://www.shop.aeddict.com/search?type=article,product&q=" + tags, "_self", true);
		} else {
	...
	private void displayQuestion(final VerticalLayout layout, Panel mainPanel, QuestionNode node) {
		...
		CssLayout questionLayout = new CssLayout() {
			...
			protected String getCss(Component c){
				...
				if(imageNames.size() == 1){
					sportImage = "images/" + collection +"/" + imageNames.get(0).getImage();
				} else if(tags.toLowerCase().contains(weiblich)) {
					sportImage = "images/" + collection +"/" + imageNames.get(0).getImage();
				} else {
					sportImage = "images/" + collection +"/" + imageNames.get(1).getImage();
				}
				
				return "background: transparent url(" + sportImage;
		...
		for (QuestionNodeData nodeData : data) {
			...
			button.addStyleName("button_question");
			button.setSizeFull();
		...
		/*questionLayout.addComponent(question, "top:10%; left:15%;");*/
	...
	private HorizontalLayout addHeader(){
		...
		Link linkSortiment = new Link("Sortiment", new ExternalResource("https://shop.aeddict.com/collections"));
		linkSortiment.addStyleName("sortiment");
		links.addComponent(linkSortiment);
		
		
		Link linkFragen = new Link("Fragen", new ExternalResource("htttp://aeddict.com"));
		linkFragen.addStyleName("sortiment");
		links.addComponent(linkFragen);
		links.addStyleName("links");
		
		header.addComponent(links);
		...
		searchImage.addClickListener(event ->Page.getCurrent().open("http://www.shop.aeddict.com/search?q=", "_self", true));
		shopifyIcons.addComponent(searchImage);
	
		
		account.addClickListener(e -> Page.getCurrent().open("http://shop.aeddict.com/account", "_self", true));
		account.setWidth("20px");
		account.addStyleName("shopifyIcon");
		shopifyIcons.addComponent(account);
		
		shoppingcart.addClickListener(e -> Page.getCurrent().open("https://shop.aeddict.com/cart", "_self", true));
		...
		header.setComponentAlignment(links, Alignment.BOTTOM_CENTER);
		header.setComponentAlignment(shopifyIcons, Alignment.TOP_LEFT);
		header.setExpandRatio(links, 1.0f);
	...
	private VerticalLayout addFooter(){
		...
		links.setWidth("40%");
		Link linkAeddict = new Link("Über aeddict", new ExternalResource("https://shop.aeddict.com/pages/about-aeddict"));
		Link linkAGB = new Link("AGB", new ExternalResource("https://shop.aeddict.com/pages/agb"));
		Link Datenschutz = new Link("Datenschutz", new ExternalResource("https://shop.aeddict.com/pages/datenschutz"));
		Link Kontakt = new Link("Kontakt", new ExternalResource("https://shop.aeddict.com/pages/kontakt"));
